 ...
 *
 * 变量格式：
 *   YYB_SERVER=YYB-Go地址@微信账号标识
 *   多账号一行一个，地址可写 yyb-go:8000 或完整 http(s) URL。
 *
 * 说明：
 *   1) 签名算法已完整内嵌 sign.js（X-Sign-For + appKey）
 *   2) 每个账号即时从 YYB-Go-Enhanced 获取 code 和手机号授权数据并登录
 *   3) 登录成功后缓存 refreshToken，并在业务请求前自动续期
 *   4) 自动流程：任务列表 -> 尝试完成未完成任务 -> 签到 -> 抽奖 -> 查询积分前后变化
 */
...
const API = {
  ...
  LOGIN_4_MINIAPP: '/weixintv/oauth/login4MiniAPPByPhone',
  GET_MINIAPP_USER_INFO: '/weixintv/oauth/getMiniAppUserInfo',
...
const CONSTS = {
  ...
  WX_APP_ID: 'wxf488d623a17cd7b5',
  WX_MINI_ID: '105',
  WX_FEATURE_CODE: '86100300000100100000fffe',
};

const CACHE_FILE = path.join(__dirname, 'hsaj_cache.json');

const RAW_YYB_SERVER = String(process.env.YYB_SERVER || '').trim();
...
async function sendNotifySafe(title, content) {
  ...
  try {
    let sender = null;
    for (const modulePath of ['./sendNotify', '../sendNotify']) {
      try {
        const mod = require(modulePath);
        sender = mod?.sendNotify || mod;
        if (typeof sender === 'function') break;
      } catch (_) {
        // 兼容脚本位于青龙根目录或订阅子目录的两种布局。
      }
    }
...
function parseYYBServer(raw) {
  return String(raw || '')
    ...
    .map((line, idx) => {
      ...
      let server = line.slice(0, at).trim();
      const ref = line.slice(at + 1).trim();
      if (!/^https?:\/\//i.test(server)) server = `http://${server}`;
      server = server.replace(/\/+$/, '');
      return server && ref ? { server, ref, remark: `账号${idx + 1}` } : null;
    })
    .filter(Boolean);
}

async function yybRequest(entry, endpoint) {
  const { data } = await request('POST', `${entry.server}${endpoint}`, {
    headers: { 'content-type': 'application/json' },
    data: { ref: entry.ref, app_id: CONSTS.WX_APP_ID },
    timeout: 25000,
  });
  const result = data?.data?.result;
  if (Number(data?.code ?? -1) !== 0 || !result || typeof result !== 'object') {
    throw new Error(`${endpoint}失败: ${data?.msg || data?.message || safeJsonPreview(data)}`);
  }
  return result;
}

async function loginByYYB(entry) {
  const codeResult = await yybRequest(entry, '/wxapp/getCode');
  const loginCode = String(codeResult.code || '').trim();
  if (!loginCode) throw new Error('YYB-Go返回缺少微信code');
  console.log('  - YYB-Go获取微信code成功');

  const phoneResult = await yybRequest(entry, '/wxapp/getPhoneNumber');
  const phoneCode = String(phoneResult.code || '').trim();
  const encryptedData = String(phoneResult.encryptedData || phoneResult.encrypted_data || '').trim();
  const iv = String(phoneResult.iv || '').trim();
  if (!phoneCode || !encryptedData || !iv) {
    throw new Error('YYB-Go返回的手机号授权数据不完整');
  }
  console.log('  - YYB-Go获取手机号授权数据成功');

  const unsignedBody = {
    sid: CONSTS.WX_MINI_ID,
    code: loginCode,
    phoneCode,
    encryptedData,
    iv,
    cloudId: String(phoneResult.cloud_id || phoneResult.cloudId || ''),
    appId: CONSTS.WX_APP_ID,
    appid: CONSTS.WX_APP_ID,
    wxAppId: CONSTS.WX_APP_ID,
    miniId: CONSTS.WX_MINI_ID,
    miniAppId: CONSTS.WX_MINI_ID,
    featureCode: CONSTS.WX_FEATURE_CODE,
  };
  const body = Object.assign({}, unsignedBody, {
    sign: sign(unsignedBody, { appKey: CONSTS.APP_KEY, postAndJSON: true }),
  });
  const { data } = await request('POST', `${HOSTS.WXTV}${API.LOGIN_4_MINIAPP}`, {
    headers: { 'content-type': 'application/json' },
    data: body,
    timeout: 25000,
  });
  if (Number(data?.resultCode ?? -1) !== 0 || !data?.token || !data?.refreshToken || !data?.customerId) {
    const detail = data?.errorDesc || data?.message || data?.msg || safeJsonPreview(data);
    throw new Error(`海信登录失败: code=${data?.resultCode ?? data?.errorCode ?? '-'} msg=${detail}`);
  }
  console.log('  - 海信业务登录成功');
  return {
    ...
    remark: entry.remark,
    deviceId: '',
    tokenCreateTime: Number(data.tokenCreateTime || 0),
    tokenExpiredTime: Number(data.tokenExpiredTime || 0),
    refreshTokenExpiredTime: Number(data.refreshTokenExpiredTime || 0),
    yybRef: entry.ref,
  };
}
...
function parseAccounts(raw) {
  return raw
    ...
    .map((line, idx) => {
      if (line.startsWith('{')) {
        try {
          ...
          const explicitToken = String(obj.token || obj.accessToken || '').trim();
          if (!refreshToken && !explicitToken) return null;
          return {
            token: explicitToken || buildAccessTokenByRefreshToken(refreshToken),
            refreshToken,
    ...
    .filter((x) => x && (x.refreshToken || x.token));
}
...
async function refreshTokenIfPossible(acc) {
  ...
  try {
    ...
    if (topCode !== 0 || bodyCode !== 0 || !body?.token) {
      throw new Error(`refresh失败: top=${topCode} body=${bodyCode} msg=${body?.msg || data?.msg || ''} raw=${safeJsonPreview(data)}`);
    }
...
async function getLoginKeyByAccessToken(accessToken) {
  ...
  if (code !== 0 || !loginKey) {
    throw new Error(`getLoginKey失败: code=${code} msg=${body?.message || body?.msg || data?.message || data?.msg || ''} raw=${safeJsonPreview(data)}`);
  }
  return loginKey;
}
...
async function main() {
  if (!RAW_YYB_SERVER) {
    throw new Error('未设置变量 YYB_SERVER');
  }

  const yybEntries = parseYYBServer(RAW_YYB_SERVER);
  if (!yybEntries.length) {
    throw new Error('YYB_SERVER解析失败，请按“地址@微信账号标识”每行一个账号配置');
  }

  const cache = loadCache();
  console.log(`${SCRIPT_NAME} 启动，YYB-Go账号数: ${yybEntries.length}`);
  ...
  for (let i = 0; i < yybEntries.length; i++) {
    const entry = yybEntries[i];
    console.log(`\n========== [${i + 1}/${yybEntries.length}] ${entry.remark} ==========`);
    let merged = { remark: entry.remark, yybRef: entry.ref };

    try {
      const fresh = await loginByYYB(entry);
      merged = mergeWithCache(fresh, cache);
      const result = await runOneAccount(merged, i + 1, yybEntries.length);
      ok += 1;
      notifyLines.push(
