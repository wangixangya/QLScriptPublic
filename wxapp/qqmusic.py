...
# -*- coding: utf-8 -*-
# name: QQ音乐
# cron: 5 0,8,10,12,16,20,22 * * *
"""
QQ音乐签到 v1.2.0（YYB-Go-Enhanced 直连版）

功能：自动执行 QQ音乐小程序绿钻成长值签到、金币签到、每日任务、抽奖和红包雨，支持多账号执行，执行结束后统一聚合推送。

配置说明：
1. YYB-Go-Enhanced 账号（与你现有青龙配置一致）：
   YYB_SERVER                                       必填，每行一个 地址@账号ref
   - 青龙与 YYB-Go 同属 qinglong_default 网络时使用：yyb-go:8000@账号ref
   - 多账号一行一个；脚本会逐行选择对应服务地址取 code

2. 账号变量：
   WX_ID                                            可选，账号白名单
   - 不填则使用 YYB_SERVER 中的全部账号
   - 多账号支持使用 &、英文逗号、中文逗号或换行分隔
   - 示例：wxid_a&wxid_b&wxid_c 或 wxid_a,wxid_b,wxid_c

3. 通知：
   自动调用青龙自带 notify.py 的 send(title, content)，通知失败不影响业务退出码。
   若青龙通知模块不可用，则兼容使用 PUSH_PLUS_TOKEN / PLUSPLUS_TOKEN。
...
5. 青龙任务建议：
   名称：QQ音乐签到
   命令：python3 qqmusic.py
   定时：5 0,8,10,12,16,20,22 * * *（覆盖红包雨及 9/10 点定时任务窗口）
...
# 兼容青龙根目录、仓库子目录和本地运行时的通知模块查找路径
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
for notify_path in ("/ql/data/scripts", "/ql/scripts"):
...
# ====================== YYB-Go-Enhanced 多行账号 ======================
ACCOUNT_NAMES: Dict[str, str] = {}
ACCOUNT_SERVERS: Dict[str, str] = {}


def load_accounts() -> List[str]:
    """解析多行 YYB_SERVER=服务地址@账号ref，并应用可选 WX_ID 白名单。"""
    refs: List[str] = []
    for line_no, raw in enumerate(os.getenv("YYB_SERVER", "").splitlines(), 1):
        item = raw.strip()
        if not item:
            continue
        if "@" not in item:
            print(f"❌ YYB_SERVER 第 {line_no} 行缺少 @，已跳过")
            continue
        server, ref = item.rsplit("@", 1)
        server, ref = server.strip().rstrip("/"), ref.strip()
        if not server or not ref:
            print(f"❌ YYB_SERVER 第 {line_no} 行地址或账号ref为空，已跳过")
            continue
        if not server.startswith(("http://", "https://")):
            server = "http://" + server
        if ref not in ACCOUNT_SERVERS:
            ACCOUNT_SERVERS[ref] = server
            ACCOUNT_NAMES[ref] = f"账号{len(refs) + 1}"
            refs.append(ref)

    raw_filter = os.getenv("WX_ID", "").strip()
    if raw_filter:
        allowed = {x.strip() for x in re.split(r"[&,，\n]", raw_filter) if x.strip()}
        refs = [ref for ref in refs if ref in allowed]
    if refs:
        print(f"✅ 已从 YYB_SERVER 载入 {len(refs)} 个账号")
    return refs
...
def dispatch_notify(title: str, content: str) -> None:
    if ql_notify_send:
        try:
            ...
            print("✅ [通知] 青龙 notify.py 调用完成")
            return
        except Exception as exc:
            print(f"⚠️ [通知] 青龙 notify.py 调用异常: {exc}")

    send_pushplus(title, content)


def get_code(wxid: str) -> str | None:
    """直接调用 YYB-Go-Enhanced /wxapp/getCode 获取一次性微信 code。"""
    server = ACCOUNT_SERVERS.get(wxid, "")
    if not server:
        print(f"❌ [授权] 未找到账号 {mask(wxid)} 对应的 YYB 服务")
        return None
    for attempt in range(1, 3):
        try:
            ...
            body = response.json()
            result = ((body.get("data") or {}).get("result") or {}) if isinstance(body, dict) else {}
            code = result.get("code") if isinstance(body, dict) and body.get("code") == 0 else None
            code = str(code or "").strip()
            if not code or code.lower() in ("null", "none"):
                msg = body.get("msg") or body.get("message") or "返回中没有有效 code"
                print(f"❌ [授权] 第 {attempt} 次 code 获取失败：{msg}")
                if attempt < 2:
                    sleep(3)
                continue
...
def run_account(index: int, total: int, wxid: str) -> Dict[str, Any]:
    ...
    if not auth:
        result["error"] = f"登录失败: {redact_sensitive(json_preview(raw_login))}"
        return result
...
def main() -> int:
    ...
    if not wxids:
        append_notify_result({
            ...
            "redPacketMsg": "-",
            "balance": "-",
            "error": "未读取到有效账号，请确认 YYB_SERVER 为多行 地址@账号ref 格式",
        })
