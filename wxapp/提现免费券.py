...
def main() -> int:
    ...
    openids = [item.strip() for item in os.environ.get("wxtxopenids", OPENID).split("&") if item.strip()]
    if not openids:
        print("未提供有效的 openid")
        return 1
...
# 包装原取码函数
for _fn in ("get_wx_code", "get_code", "smallcat", "getjscode"):
    if _fn in globals():
        _orig = globals()[_fn]
...
if _yyb_accts:
    _yyb_os.environ["wxtxopenids"] = "&".join(_yyb_accts)
