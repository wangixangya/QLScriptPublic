...
async function getCode(serverEntry) {
    ...
    try {
        ...
        if (!data || data.code !== 0 || !code) {
            console.log(server + " 获取code失败: " + JSON.stringify(data));
            return null;
        }
        console.log(server + " 获取code成功");
        return code;
    } catch (e) {
        console.log(server + " 获取code异常: " + e.message);
        return null;
    }
}
async function getPhoneCode(serverEntry) {
    ...
    try {
        ...
        const code = data && data.data && data.data.result && data.data.result.code;
        if (!data || data.code !== 0 || !code) {
            console.log(server + " 获取手机号code失败: " + JSON.stringify(data));
            return null;
        }
        console.log(server + " 获取手机号code成功");
        return code;
    } catch (e) {
        console.log(server + " 获取手机号code异常: " + e.message);
        return null;
    }
}
