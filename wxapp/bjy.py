...
def send_pushplus(title: str, content: str) -> None:
    """沿用旧函数名，实际交给青龙 notify.py；通知失败不影响任务。"""
...
def main() -> None:
    ...
    if not SERVERS:
        print("❌ 未从 yyb_go 获取到存活账号，请确认 YYB_SERVER 已配置且服务内有在线账号")
        send_pushplus(
            "♻️ 白鲸鱼旧衣服回收任务失败",
            "未从 yyb_go 获取到存活账号，请确认 YYB_SERVER 已配置且服务内有在线账号",
        )
        return
